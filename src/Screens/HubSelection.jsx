...
const HubSelection = () => {
    ...
    return (
        <section className='w-full min-h-screen flex items-center justify-center'>
            <div className="p-6 mx-auto border bg-white rounded-2xl">
  ...
  <div className="flex flex-col gap-4 my-3">
    ...
    <div className="space-y-3 max-h-60 overflow-y-auto pr-2">
      {hubs.map((hub) => (
        <label
          key={hub.name}
          className={`flex text-start items-center justify-between p-4 border rounded-xl cursor-pointer transition-all duration-200 ${
        ...
        >
          <div className="flex flex-col gap-1 pr-4">
            <span className={`text-sm font-semibold ${
              selectedHubLocal === hub.name ? "text-green-900" : "text-gray-800"
            }`}>
              {hub.name}
            </span>
            <span className={`text-xs ${
              selectedHubLocal === hub.name ? "text-green-700" : "text-gray-500"
            }`}>
              {hub.address}
            </span>
          </div>
