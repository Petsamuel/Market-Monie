...
const Sidebar = ({ isCollapsed, setIsCollapsed, isMobileOpen, setIsMobileOpen }) => {
  ...
  const sidebarClasses = `
    bg-emerald-800/85 dark:bg-gray-900/95 text-white flex flex-col h-screen fixed left-0 top-0 z-50 overflow-hidden shadow-2xl transition-all duration-300
  ...
  return (
    <div className={sidebarClasses}>
      ...
      <div className={`p-6 shrink-0 relative z-10 flex items-center ${isCollapsed ? "justify-center" : "justify-between"} bg-white dark:bg-gray-800 border-b dark:border-gray-700 transition-colors mb-2`}>
        {!isCollapsed && (
          <img src="/market-monie.png" alt="Market Monie" className="h-8 w-auto transition-all animate-in fade-in" />
        )}
        <button
          onClick={() => setIsCollapsed(!isCollapsed)}
          className={`hidden lg:flex h-8 w-8 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg items-center justify-center text-emerald-800 dark:text-emerald-400 transition-all ${isCollapsed ? 'rotate-180' : ''}`}
        >
          <FiChevronLeft size={20} />
        </button>
        <button
          onClick={() => setIsMobileOpen(false)}
          className="lg:hidden h-8 w-8 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg flex items-center justify-center text-emerald-800 dark:text-emerald-400"
        >
          <FiX size={20} />
        </button>
      </div>
            ...
            {item.hasSubmenu && !isCollapsed ? (
              <div className="mb-2">
                <button
                  ...
                  className={`flex items-center justify-between w-full px-4 py-3 rounded-xl transition-all duration-300 hover:bg-white/10 group ${isLoanMenuOpen ? "text-emerald-400 bg-white/10" : "text-emerald-100/70"
                    }`}
                >
                  ...
                  <div className="mt-1 ml-4 border-l border-white/10 pl-6 space-y-1 animate-in slide-in-from-top-2 duration-300">
                    {item.submodules.map((sub) => (
                      <NavLink
                        key={sub.name}
                        to={sub.path}
                        className={({ isActive }) => `block px-3 py-2 text-[13px] rounded-lg transition-all ${isActive ? "text-emerald-400 font-bold bg-white/5" : "text-emerald-100/40 hover:text-emerald-100"
                          }`
                        }
                      >
                        {sub.name}
      ...
      <div className={`p-3 border-t border-white/10 space-y-1 mt-auto relative z-10 ${isCollapsed ? "items-center" : ""}`}>
        {bottomItems.map((item) => (
          <NavLink
            ...
            to={item.path}
            title={isCollapsed ? item.name : ""}
            className={({ isActive }) => `flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-300 ${isActive ? "text-emerald-400 bg-white/10" : "text-emerald-100/50 hover:text-white hover:bg-white/10"
              } ${isCollapsed ? "justify-center px-0" : ""}`
            }
          >
