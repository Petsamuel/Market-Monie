...
import { globalUserData, transactionsData } from '../../store/Data';
...
const TransactionDetails = () => {
    ...
    const transaction = transactionsData.find(t => t.id === parseInt(txId)) || transactionsData[0];

    const bankList = ["PalmPay", "Opay", "Kuda Bank", "Access Bank", "GTBank", "Zenith Bank", "First Bank of Nigeria", "Fidelity Bank"];
    const seededId = transaction.id * 7; // Seed for deterministic "random" data

    const receiptData = {
        senderName: transaction.state === 'inflow' ? (transaction.title.replace('FIP:', '').split('...')[0].trim()) : globalUserData.username,
        senderBank: bankList[seededId % bankList.length],
        senderPhone: `8${seededId % 7}3****${200 + seededId}`,
        amount: parseFloat(transaction.amount.replace(/[-+ ₦,]/g, "")).toLocaleString(undefined, { minimumFractionDigits: 2 }),
        status: transaction.status,
        creditedTo: transaction.state === 'inflow' ? "Available Balance" : "External Recipient",
        remark: transaction.title,
        transactionType: transaction.title.includes("Loan") ? "Loan Disbursement" :
            transaction.title.includes("Repayment") ? "Loan Repayment" : "Bank Transfer",
        transactionNo: `260517${(seededId * 12345).toString().padStart(12, '0')}`,
        transactionDate: transaction.displayDate,
        sessionId: `100033${(seededId * 987654).toString().padStart(20, '0')}`,
        category: transaction.state === 'inflow' ? "Deposit" : "Payment"
    };
